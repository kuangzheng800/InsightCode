...
for line in file:
    ...
    time1 = line[line.find("[")+1:line.find("]")].replace('\t','').replace('-0400','').replace(' ','')
    item1 = ''.join(re.findall('"([^"]*)"',line)).replace('\t','')
    item1 = ''.join(item1.split(' ')[1:]).replace('HTTP/1.0', '')
    ...
    try:
        timestruct1 = datetime.datetime.strptime(time1,'%d/%b/%Y:%H:%M:%S') # if time of visit can't be identified
...
if len(df_goup_address) >= 10:
    Series.to_csv(df_goup_address[0:10],bsdir+'log_output/hosts.txt',header=False,index=True)
else:
    Series.to_csv(df_goup_address,bsdir+'log_output/hosts.txt',header=False,index=True)
...
df_suc_all = df[['item','size']]
df_suc_all['size'] = df_suc_all['size'].apply(lambda x:x.replace('-','0'))  # failed file request will be assigned to a size of 0
df_suc_all['size'] = df_suc_all['size'].apply(lambda x:int(x))

print('Done!')
print('In process...')
df_group_sig_all = df_suc_all.groupby('item')
resource_all = df_group_sig_all.agg(sum)
resource_all = resource_all.sort_values('size',ascending=False)
print('Top 10 resources of bandwidth use is written to ~/log_output/resources.txt\n')
if len(resource_all) >= 10:
    Series.to_csv(resource_all.index[0:10],bsdir+'log_output/resources.txt',header=False,index=False)
Series.to_csv(resource_all.index, bsdir + 'log_output/resources.txt', header=False,index=False)
...
print('Top 10 resources of bandwidth use is written to ~/log_output/resources_no401.txt\n')
if len(resource) >= 10:
    Series.to_csv(resource.index[0:10],bsdir+'log_output/resources_no401.txt',header=False,index=False)
Series.to_csv(resource.index, bsdir + 'log_output/resources_no401.txt', header=False,index=False)
...
start = df.loc[0,'time']
start = datetime.datetime.strptime(start.replace('\t','').replace(' ',''),'%d/%b/%Y:%H:%M:%S')
...
sec_list_overlap = list()
n_visit_overlap = list()
while x < last:
    i = time2index(df,x)
    df1 = subset_time_forward(df,i,hour)
    sec_list_overlap.append(x)
    n_visit_overlap.append(len(df1))
    x += second
time_visit_overlap = DataFrame({'sec':sec_list_overlap,'n_visit':n_visit_overlap})
time_visit_overlap = time_visit_overlap.sort_values('n_visit',ascending=False)
busy_time_overlap = time_visit_overlap[0:10]

busy_time_overlap_list = list()
for i in range(len(busy_time_overlap)):
    time = start+datetime.timedelta(seconds=float(busy_time_overlap.loc[i,'sec']))
    time = time.strftime('%d/%b/%Y:%H:%M:%S')
    busy_time_overlap_list.append(time+' -0400')

temp = DataFrame({'sec':busy_time_overlap_list,'n_visit':busy_time_overlap.n_visit})
temp = temp[['n_visit','sec']]
temp.to_csv(path_or_buf=bsdir+'log_output/hours.txt',header=False,sep=',',index=False)

################################################
# deciding the top 10 hours of vistis without overlapping

x = 0 # re-initiate scanning
sec_list = list() # the output lists
n_visit = list()
while x+hour < last or x == 0: # end when the end of the time window reaches the end of the data's time span
...
################################################
# transfering seconds elapsed to datetime and then to string format
busy_time_struct = list()
for i in range(len(busy_time_visit)):
    time = start+datetime.timedelta(seconds=float(busy_time_visit.loc[i,'sec']))
    time = time.strftime('%d/%b/%Y:%H:%M:%S')
    busy_time_struct.append(time+' -0400')
busy_time = DataFrame({'Number of visits': busy_time_visit['n_visit'],'Starting time of the hour': busy_time_struct})
busy_time = busy_time[['Starting time of the hour','Number of visits']]
print('Top 10 resources of busy hours use is written to ~/log_output/hours.txt\n')
busy_time.to_csv(path_or_buf=bsdir+'log_output/hours_no_overlap.txt',header=False,sep=',',index=False)
